...
Еще ниже показаны изменения, сделанные в этом коммите по сравнению с предыдущим.
...
Такой формат записи изменений называется **diff**.

Выберите в списке коммит «Построили гистограммы для длины клюва» и изучите изменения.
Обратите внимание, что по умолчанию Git анализирует строки целиком, а не отдельные слова или символы.
Поэтому получился следующий дифф: старая вторая строка была удалена, новая вторая строка добавлена (см. номера строк слева), хотя мы всего лишь добавили `x <- ` в ее начало.

![](./img/02-04-03-diff.png){width=690px}

Даже если вы работаете один, полезно иметь под рукой историю изменений.
