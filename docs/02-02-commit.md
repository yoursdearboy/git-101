...
Git отслеживает изменения в файлах, находящихся внутри рабочей папки.
...
Коммиты нужно создавать вручную, выбирая, какие файлы вы хотите зафиксировать.
...
Так как мы еще не создали ни одной версии, то в списке выводятся оба файла, созданных с проектом: `.gitignore` и `hello-git.Rproj`
...
это означает, что они добавлены в **индекс (staging)**,
промежуточную область для выбора файлов, которые будут зафиксированы.
Остальные файлы останутся в рабочей папке, но в коммит не попадут.
...
После этого введите краткое описание сделанных изменений в поле [Commit message]{.figtext}.
В нашем примере был создан проект, это и будет описанием.
...
Закройте окно с сообщением и окно создания коммита и вернитесь к RStudio.
...
Схематично процесс работы с Git показан на диаграмме.
