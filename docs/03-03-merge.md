...
[Откройте журнал](#git-log) и разверните [Source Control Graph]{.figtext}, чтобы посмотреть все ветки.
На диграмме наглядно видно почему это называется веткой.

![Source Control Graph перед слиянием: ветки main и boxplot расходятся от общего коммита. Подготовьте состояние conflict и выполните git merge --abort]()
...
`out/penguins-hist.png` и `out/penguins-boxplot.png`,
но лишь частично для документа `paper.qmd`.

![Source Control после команды git merge boxplot: файлы графиков объединены автоматически, а paper.qmd отмечен как конфликтующий. Команда подготовки: python3 bin/create_course_project.py /tmp/git-course-conflict --state conflict]()
...
в том числе обновившийся файл `out/penguins-boxplot.png`, который теперь содержит подпись оси.
Обычно сообщение коммита о слиянии веток содержит следующий текст «Merge branch название-ветки», но можете придумать свое.

![Source Control перед коммитом слияния: paper.qmd и обновлённый out/penguins-boxplot.png добавлены в индекс. Подготовьте состояние conflict, разрешите конфликт по заданию и добавьте файлы в индекс]()
...
![Source Control Graph после слияния: ветки boxplot и main соединены коммитом слияния. Команда подготовки: python3 bin/create_course_project.py /tmp/git-course-merged --state merged]()
