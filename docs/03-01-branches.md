...
До этого мы создавали версии только последовательно, чтобы записать историю изменений.
...
Таким образом участники (collaborators) могут создавать и обмениваться различными вариантами изменений.
...
Для этого используются так называемые ветви (branch, бранч).
...
Из журнала приходит ответ Рецензента 2, в котором он просит заменить гистограмму на ящик с усами.
...
**Ошибка при переключении ветвей.**

Git может отказаться переключать ветви, если это приведет к потере данных.

Добавьте в файл `penguins.R` несколько любых строк и сохраните.
Попробуйте переключиться назад на ветку `boxplot`.
Вы получите сообщение как на скриншоте ниже.

![](./img/03-02-03-error.png){width=640px}

*(надо осмысленных и закомитить)*
