...
quarto_render() {
    quarto render homework_notebook.qmd
}
```
...
cp $ASSETS/workshop_notebook.qmd homework_notebook.qmd
git add .
...
# quarto_render не выйдет - хорошо
git add .
...
**👷‍♂️ УЧАСТНИКИ** делают свои задания в ветках и **‼️ собирают qmd**.
...
Делаем `pull` — Positron сделает и `fetch`.
...
cp $ASSETS2/homework_notebook.qmd homework_notebook.qmd
git add .
